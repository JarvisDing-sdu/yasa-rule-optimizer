...
进入 `/rule-workshop` 页面，从 CVE 描述自动生成可直接用于扫描的 YASA 检测规则。

**操作流程：**

1. **搜索 CVE**：在左侧面板按语言、漏洞类型或关键词检索 GitHub Advisory Database，支持 SQL 注入、命令注入、SSRF、路径穿越、XSS、反序列化、XXE、模板注入等 23 种漏洞类型，可查看每条 CVE 的详情、影响版本和修复方案
2. **生成规则**：勾选目标 CVE，选择或新建个人规则集，点击「生成」，后端自动运行四阶段管道
3. **管理规则**：在右侧面板对生成的规则启用、禁用或删除；也可克隆官方内置规则集后按需修改
4. **应用扫描**：在扫描控制台勾选个人规则集，即可在正常扫描流程中叠加使用自定义规则

**四阶段生成管道：**

```
Stage 1  LLM 分析  →  提取漏洞的 source、sink、传播条件、数据流特征
Stage 2  LLM 生成  →  输出 YASA 规则 JSON（checkerIds / sources / sinks / bridge）
Stage 3  真实评估  →  将规则写入临时配置 → 调 YASA 引擎扫描测试集 → 解析 SARIF → 计算 TP/FP/FN/Precision/Recall/F1
Stage 4  LLM 优化  →  LLM 读取具体误报行号和漏报数量，精准调整 fsig/args/attribute/scope
          └─ 迭代 Stage 3→4，直到 F1 ≥ 0.85 或连续 3 轮无改进
```

Stage 3 是关键所在：传统方案让 LLM 自评生成结果，无法知道规则在真实引擎上的表现。本项目将规则投入实际 YASA 扫描，LLM 在 Stage 4 拿到的是「第 25 行 `os.system()` 被误报、预期的 2 处 sink 未检出」这样的具体数据，而非模糊的"规则可能有问题"，使优化更具针对性。

扫描结果按规则内容哈希缓存，相同规则不重复扫描。若项目中没有配置测试集（`test-{lang}-cases/expected.json`），Stage 3 自动跳过，降级为快速模式（仅 Stage 1+2 + LLM 自评）。
