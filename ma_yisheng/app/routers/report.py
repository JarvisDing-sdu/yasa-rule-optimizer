...
@router.post("/reports/{report_path:path}/logic-audit", summary="业务逻辑漏洞审计")
def logic_audit_endpoint(report_path: str, user: Dict = Depends(get_current_user)):
    _assert_report_owner(report_path, user["user_id"])
    from llm import audit_business_logic
    from report import get_structured_findings
    rd = Path(report_path)
    if not rd.is_dir():
        raise HTTPException(status_code=404, detail="报告目录不存在")
    ...
    if report_json.exists():
        import json as _j
        data = _j.loads(report_json.read_text(encoding="utf-8"))
        scan_path = data.get("scan_path") or data.get("project_path")
    if not scan_path or not os.path.isdir(scan_path):
        raise HTTPException(status_code=400, detail="找不到原始扫描路径")
    findings = get_structured_findings(report_path)
    result = audit_business_logic(scan_path, findings)
    if not result:
        raise HTTPException(status_code=503, detail="LLM 调用失败或源码为空")
    return {"ok": True, "report_path": report_path, **result}
