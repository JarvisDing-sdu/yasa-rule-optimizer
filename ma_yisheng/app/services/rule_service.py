        ...
        for scene in ("minimal", "full"):
            ...
            if fpath.exists():
                ...
                sets.append({
                    ...
                    "name": f"内置 {lang.title()} {scene.title()}",
                    "description": f"YASA 内置 {lang} {scene} 规则库",
...
def get_official_rule_set_detail(rule_set_id: str):
    ...
    return {
        'id': rule_set_id,
        'name': f'内置 {lang.title()} {scene.title()}',
        'description': f'YASA 内置 {lang} {scene} rules',
        'source_type': 'official',
        'lang': lang,
        'scene': scene,
...
def _first_sink_attribute(rule: Dict) -> str:
    ...
    for entries in sinks.values():
        ...
        for entry in entries:
            if isinstance(entry, dict) and entry.get("attribute"):
                return str(entry["attribute"])
    return ""


def _first_checker_id(rule: Dict) -> str:
    checker_ids = rule.get("checkerIds") or []
    if isinstance(checker_ids, list) and checker_ids:
        return str(checker_ids[0])
    return "generated_rule"


def _normalize_rule_metadata(rule: Dict, index: int = 0) -> Dict:
    meta = rule.setdefault("metadata", {})
    if not isinstance(meta, dict):
        meta = {}
        rule["metadata"] = meta

    case_id = str(meta.get("caseId") or "").strip()
    attribute = _first_sink_attribute(rule)
    checker_id = _first_checker_id(rule)
    suffix = str(index + 1)

    if not meta.get("ruleId"):
        base = case_id or attribute or checker_id
        meta["ruleId"] = f"{base}_{suffix}" if case_id else f"{base}_{suffix}"

    if not meta.get("name"):
        meta["name"] = attribute or case_id or meta["ruleId"]

    if not meta.get("description"):
        sink_count = sum(len(v) for v in (rule.get("sinks") or {}).values() if isinstance(v, list))
        source_count = sum(len(v) for v in (rule.get("sources") or {}).values() if isinstance(v, list))
        meta["description"] = (
            f"{meta['name']} rule generated by rule workshop"
            f" ({source_count} sources -> {sink_count} sinks)"
        )
...
def add_rules_to_set(user_id: int, rule_set_id: int,
    ...
    try:
        ...
        for index, rule in enumerate(rules):
            rule = _normalize_rule_metadata(rule, index)
