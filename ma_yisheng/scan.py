...
async def scan_upload(
    ...
    # Parse rule_set_ids from JSON string
    rids = None
    if rule_set_ids:
        try:
            import json as _json
            rids = _json.loads(rule_set_ids)
        except Exception:
            pass
    ...
    background_tasks.add_task(
        run_scan_background,
        task_id, extract_dir, lang, scene, timeout, favorite, temp_dir, user["user_id"], engine, rids,
    )
    return {"task_id": task_id}
...
def list_tasks(user: Dict = Depends(get_current_user)):
    ...
    conn = _db()
    rows = conn.execute(
        "SELECT task_id, status, scan_path, lang, created_at, progress FROM scan_tasks WHERE user_id=? ORDER BY created_at DESC LIMIT 100",
        (user["user_id"],)
    ).fetchall()
    conn.close()
